...
export default function Edit({ attributes, setAttributes }) {
  ...
  const updateQuickLink = (index, key, value) => {
    ...
    newQuickLinks[index][key] = value;
    setAttributes({ quickLinks: newQuickLinks });
  };
  ...
  const removeQuickLink = (index) => {
    const newQuickLinks = quickLinks.filter((_, i) => i !== index);
    setAttributes({ quickLinks: newQuickLinks });
  };
  ...
  const moveQuickLink = (index, direction) => {
    ...
    if (newIndex >= 0 && newIndex < quickLinks.length) {
      [newQuickLinks[index], newQuickLinks[newIndex]] = [newQuickLinks[newIndex], newQuickLinks[index]];
      setAttributes({ quickLinks: newQuickLinks });
    }
  };
    ...
    <>
      <InspectorControls>
        ...
        <PanelBody title={__('Logo', 'webdune-blocks')} initialOpen={true}>
          ...
          <TextControl
            label={__('Tagline', 'webdune-blocks')}
            value={tagline}
            onChange={(value) => setAttributes({ tagline: value })}
            help={__('Text below the logo', 'webdune-blocks')}
          />
        </PanelBody>

        {/* Quick Links */}
        <PanelBody title={__('Quick Links', 'webdune-blocks')} initialOpen={false}>
          {quickLinks.map((link, index) => (
            <div key={index} style={{ marginBottom: '20px', padding: '12px', border: '1px solid #ddd', borderRadius: '4px' }}>
              ...
              />
              <TextControl
                label={__('Link URL', 'webdune-blocks')}
                value={link.url}
          ...
          <Button onClick={addQuickLink} variant="secondary">
            {__('Add Link', 'webdune-blocks')}
          </Button>
        </PanelBody>

        {/* Contact Information */}
        <PanelBody title={__('Contact Information', 'webdune-blocks')} initialOpen={false}>
          <TextControl
            label={__('Office Hours', 'webdune-blocks')}
            value={officeHours}
            onChange={(value) => setAttributes({ officeHours: value })}
          />
          <TextControl
            label={__('Email', 'webdune-blocks')}
            value={email}
            onChange={(value) => setAttributes({ email: value })}
            type="email"
          />
          <TextControl
            label={__('Phone', 'webdune-blocks')}
            value={phone}
            onChange={(value) => setAttributes({ phone: value })}
            type="tel"
          />
          <TextareaControl
            label={__('Address', 'webdune-blocks')}
            value={address}
            onChange={(value) => setAttributes({ address: value })}
            help={__('Use <br> for line breaks', 'webdune-blocks')}
          />
        </PanelBody>

        {/* Newsletter Settings */}
        <PanelBody title={__('Newsletter', 'webdune-blocks')} initialOpen={false}>
          <ToggleControl
            label={__('Show Newsletter Signup', 'webdune-blocks')}
          ...
          {showNewsletter && (
            <>
              <TextControl
                label={__('Newsletter Heading', 'webdune-blocks')}
                value={newsletterHeading}
                onChange={(value) => setAttributes({ newsletterHeading: value })}
              />
              <TextControl
                label={__('Newsletter Text', 'webdune-blocks')}
        ...
        {/* Social Links */}
        <PanelBody title={__('Social Media Links', 'webdune-blocks')} initialOpen={false}>
          {socialLinks.map((link, index) => (
            <div key={index} style={{ marginBottom: '20px', padding: '12px', border: '1px solid #ddd', borderRadius: '4px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <strong>{__('Social Link', 'webdune-blocks')} {index + 1}</strong>
                <Button
                  icon="trash"
              ...
              <SelectControl
                label={__('Platform', 'webdune-blocks')}
                value={link.platform}
                options={[
          ...
          <Button onClick={addSocialLink} variant="secondary">
            {__('Add Social Link', 'webdune-blocks')}
          </Button>
        </PanelBody>
      </InspectorControls>
            ...
            <div className="container-medium">
              <div className="footer2_top-wrapper">
                ...
                <div className="footer_logo-wrap">
                  {logo?.url && (
                    <img src={logo.url} alt={logo.alt || ''} className="footer2_logo" />
                  )}
                  {!logo?.url && (
                    <div style={{ padding: '20px', background: '#f5f5f5', textAlign: 'center' }}>
                      {__('Logo', 'webdune-blocks')}
                    </div>
                  )}
                ...
                <div className="footer2_link-column">
                  <div className="margin-bottom margin-xsmall">
                    <div className="text-weight-semibold"><strong>{__('Quick Links', 'webdune-blocks')}</strong></div>
                  </div>
                  <div className="footer2_link-list">
                    {quickLinks.map((link, index) => (
                      <a key={index} href={link.url} className="footer2_link">{link.text}</a>
                    ))}
                  </div>
                </div>
                ...
                <div className="footer2_link-column">
                  <div className="footer_office-hours">
                    <div className="text-weight-semibold"><strong>{__('Office hours', 'webdune-blocks')}</strong></div>
                    <div>{officeHours}</div>
                  </div>
                  <div className="footer_info-list">
                    <a href={`mailto:${email}`} className="text-style-link">{email}</a>
                    <div>
                      {__('Need help?', 'webdune-blocks')}<br />
                      <a href={`tel:${phone.replace(/\s/g, '')}`} className="text-style-link">{phone}</a>
                    </div>
                ...
                {showNewsletter && (
                  <div className="footer2_right-wrapper">
                    ...
                    <div className="footer2_social-list">
                      {socialLinks.map((link, index) => (
                        <a key={index} href={link.url} className="footer2_social-link">
